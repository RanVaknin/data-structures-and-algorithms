...
const totalCharacters = (arr) => {
  ...
  for (let i = 0; i < arr.length; i++) {
    ...
    if (arr[i].children.length) {
      ...
      children.push(arr[i].children)
    }
  }
  children = children.toString().split(',')
  console.log(children)
  return children.length + names.length + spouse.length
};

/* ------------------------------------------------------------------------------------------------
CHALLENGE 6 - Stretch Goal

Write a function named houseSize that takes in the array of characters and creates an object for each house 
