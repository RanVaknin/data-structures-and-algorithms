...
const stepActions = (recipe) => {
  let step = recipe.steps;
  let result = [];
  for (let i = 0; i < step.length; i++) {
    let arr = step[i].split(' ')
    let storethis = arr.shift()
    result.push(storethis)
  }
  // Solution code here...
  return result;
};
...
const removeVowels = (str) => {
  ...
  for (let i = 0; i < strarray.length; i++) {
    for (let j = 0; j < vowelsArray.length; j++) {
      if (strarray[i] === vowelsArray[j]) {
        newarr.push(strarray[i]);
      }
    }
  }
  for (let i = 0; i < newarr.length; i++) {
    let removedValue = strarray.indexOf(newarr[i]);
    strarray.splice(removedValue, 1);
  }
  let end = strarray.join('');
  return end;
};
...
const extractVowels = (str) => {
  ...
  let end = strarray.join('');
  let sortedVowel = newarr.sort();
  let vowelend = sortedVowel.join('')
  let finisharray = []
  finisharray.push(end);
  finisharray.push(vowelend);
