...
const count = (target, input) => {
  let flattenArray = input.reduce((a, b) => {
    return a.concat(b)
  },[])
  let targetArray = flattenArray.filter(word => word === target)
...
const totalSum = (input) => {
  let newInput = input.reduce((a, b) => {
    return a.concat(b);
  })
  let totalNum = newInput.reduce((a, b) => { return a + b })
  return totalNum;
};
...
const divisibleByFiveTwoToThePower = (input) => {
  ...
  for (let i = 0; i < input.length; i++) {
    newArray.push(input[i].map(element => {
      if (element % 5 === 0) {
        return element
      }
    }));
  ...
  for (let i = 0; i < newArray.length; i++) {
    lastArray.push(newArray[i].filter(element => {
      return element !== undefined
    }))
  }
  let divisibleByFiveArray = [];
  for (let i = 0; i < lastArray.length; i++) {
    divisibleByFiveArray.push(lastArray[i].map(element => {
...
let findMaleAndFemale = (data) => {
  const gender = data.filter( element => {
...
let findShortest = (data) => {
  const shortest = data.map( element => element.name).sort((a,b)=>{
