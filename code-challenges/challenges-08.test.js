...
const getBaseStatGreaterThan = (arr, minBaseStat) => {
  let filtered = arr.filter(move => {
    if (move.baseStat > minBaseStat) {
      return move
    }
  })
  return filtered;
...
const evenOddNumericValues = (arr) => {
  let filteredNums = arr.filter(e => { if (typeof (e) === 'number') { return e } });
  let numValues = filteredNums.map(e => {
    if (e % 2 === 0) {
      return 'even'
    } else {
      return 'odd'
